...
#define NUMBER_XOR_KEYS 10
#define NUMBER_PBOX_SBOX_KEYS 10
...
uint16_t cypher(uint16_t _plaintext)
{
  ...
  for (int k = 0; k <= NUMBER_PBOX_SBOX_KEYS; k++)
  {
    for (int p = 0; p <= NUMBER_XOR_KEYS; p++) {
      ...
      for (int i = 0; i < 16; i++) cypher |= temp[i] << i;
      plaintext  = 0;
      for (int i = 0; i < 4; i++) plaintext |= S_BOX[k][((uint16_t)cypher >> (i * 4) & 15)] << (4 * i); //SBOX
    }
  }
  return plaintext;
}
...
uint16_t decypher(uint16_t _cypher)
{
  uint16_t cypher = _cypher;
  uint16_t arr_temp[4] = {0};
  ...
  for (int j = NUMBER_PBOX_SBOX_KEYS; j >= 0 ; j--)
  {
    for (int p = NUMBER_XOR_KEYS; p >= 0; p--)
    {
      for (int k = 0; k < 4 ; k++) {
        for (int i = 0; i < 16; i++) {
          if (((uint16_t)cypher >> (k * 4) & 15) == S_BOX[j][i] ) {
            arr_temp[k] = i;  //reerse S_BOX
            break;
          }
        }
      }
      ...
      decypher = 0;
      for (int i = 0; i < 16; i++) decypher |= arr_temp[i] << i; //unisco in una sola varibile
      cypher = decypher ^ XOR[p]; //XOR della chiave
    }
  }
  return cypher;
}
...
void loop() {
  ...
  if (Serial.available())
  {
    ...
    Serial.write(a);
    uint16_t b = cypher(a);
    Serial1.write((uint8_t)(b >> 8 & 255));
    Serial1.write((uint8_t)b & 255);
  }
  else  if (Serial1.available() >= 2) {
    uint16_t b = decypher(Serial1.read() << 8 |  Serial1.read());
    char temp = b & 255;
#if DEBUG_MODE
    Serial.print("\ncarattere decifrato: ");
    Serial.write(temp);
    ...
    if (temp == '\n')
    {
      Serial.print("Messaggio ricevuto decifrato: " + stringa);
      stringa = "";
    }
  }
}
